...
void ElfHandler::push_to(std::ofstream &stream) {
    ...
    for (unsigned int i = 0; i < numHeaders; i++)
        segmentHeaders32[i]->push_to(stream);
    for (unsigned int i = 0; i < numHeaders; i++)
        pushChars(stream, segmentHeaders32[i]->data);
}
...
void PUSH(ElfSegmentHandler *segment, unsigned int value) {
    PUSH(segment->data,value,segment->elfHandler.elfHeader.e_encoding==ELF_ENCODING_LSB);
}
...
void MOVaddr32(ElfSegmentHandler *segment, const unsigned int &addr) {
    MOVaddr32(segment->data,addr,segment->elfHandler.elfHeader.e_encoding==ELF_ENCODING_LSB);
}
void MOVaddr64(ElfSegmentHandler *segment, const unsigned long &addr) {
    MOVaddr64(segment->data,addr,segment->elfHandler.elfHeader.e_encoding==ELF_ENCODING_LSB);
}
void MOV32(ElfSegmentHandler *segment, const char* reg, const unsigned int &value) {
    MOV32(segment->data,reg,value,segment->elfHandler.elfHeader.e_encoding==ELF_ENCODING_LSB);
}
...
void JMP32(ElfSegmentHandler *segment, unsigned int value) {
    JMP32(segment->data,value,segment->elfHandler.elfHeader.e_encoding==ELF_ENCODING_LSB);
}
void JMP64(ElfSegmentHandler *segment, unsigned long value) {
    JMP64(segment->data,value,segment->elfHandler.elfHeader.e_encoding==ELF_ENCODING_LSB);
}
